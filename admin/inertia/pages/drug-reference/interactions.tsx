...
export default function DrugReferenceInteractions({ ingestStatus, rowCount }: PageProps) {
  ...
  const doSearch = useCallback(async (q: string) => {
    ...
    try {
      ...
      setPickerError(err instanceof Error ? err.message : t('drug_reference.interactions.search_failed'))
    } finally {
  ...
  return (
    <AppLayout compact>
      <Head title={t('drug_reference.interactions.page_title')} />
      ...
      <div className="p-4 max-w-7xl mx-auto">
        ...
        >
          ...
          {t('drug_reference.interactions.back_link')}
        </Link>

        <div className="mb-5">
          <h1 className="text-2xl font-bold mb-1">{t('drug_reference.interactions.heading')}</h1>
          <p className="text-sm opacity-70">
            {t('drug_reference.interactions.subheading', { max: MAX_COMPARE })}
          </p>
        </div>
        ...
        <div className="flex gap-3 items-start bg-amber-50 border border-amber-300 rounded-lg px-4 py-3 mb-6">
          ...
          <p className="text-sm text-amber-900 leading-relaxed">
            <strong>{t('drug_reference.interactions.disclaimer_bold')}</strong>{' '}
            {t('drug_reference.interactions.disclaimer_part1')} <strong>{t('drug_reference.interactions.disclaimer_not1')}</strong>{' '}
            {t('drug_reference.interactions.disclaimer_part2')} <strong>{t('drug_reference.interactions.disclaimer_not2')}</strong>{' '}
            {t('drug_reference.interactions.disclaimer_part3')}
          </p>
        </div>
        ...
        {isEmpty ? (
          ...
          <div className="border-2 border-dashed border-border-default rounded-lg p-8 text-center">
            <p className="text-lg font-semibold mb-2">{t('drug_reference.interactions.empty_title')}</p>
            <p className="mb-6 opacity-70">
              {t('drug_reference.interactions.empty_description')}
            </p>
            <Link href="/drug-reference">
              <StyledButton variant="primary" onClick={() => {}}>
                {t('drug_reference.interactions.empty_cta')}
              </StyledButton>
            </Link>
            {ingestStatus && (
          ...
          <>
            ...
            <div className="mb-6 border border-border-subtle rounded-lg p-4 bg-surface-secondary">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold text-text-primary">{t('drug_reference.interactions.picker_label')}</h2>
                {atMax && (
                  <span className="text-xs text-amber-700 bg-amber-100 border border-amber-200 rounded px-2 py-0.5">
                    {t('drug_reference.interactions.max_reached', { max: MAX_COMPARE })}
                  </span>
                )}
              </div>
                  ...
                  {entries.map((entry) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        aria-label={t('drug_reference.interactions.remove_drug_aria', { name: entry.brand_name ?? entry.generic_name })}
                    ...
                    .map((id) => (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => removeId(id)}
                          aria-label={t('drug_reference.interactions.remove_drug_id_aria', { id })}
                          className="hover:opacity-70 transition-opacity leading-none"
                        >
                          ×
                        </button>
              ...
              <input
                ...
                placeholder={t('drug_reference.interactions.search_placeholder')}
                disabled={atMax}
              ...
              {/* Picker results */}
              {pickerLoading && (
                <div className="mt-2 text-xs text-text-secondary">{t('drug_reference.interactions.searching')}</div>
              )}
              {pickerError && (
                <div className="mt-2 text-xs text-red-600">{pickerError}</div>
              )}
              {pickerSearched && pickerResults.length === 0 && !pickerLoading && (
                <div className="mt-2 text-xs text-text-secondary">{t('drug_reference.interactions.no_results', { query })}</div>
              )}
            ...
            {selectedIds.length === 0 ? (
              <div className="text-center py-12 text-text-muted border-2 border-dashed border-border-subtle rounded-lg">
                <p className="text-sm">{t('drug_reference.interactions.empty_selection')}</p>
              </div>
            ) : loadingEntries ? (
              <div className="text-center py-12 text-text-muted">
                <p className="text-sm">{t('drug_reference.interactions.loading')}</p>
              </div>
            ) : (
        ...
        <footer className="mt-8 pt-4 border-t border-border-subtle text-xs text-text-secondary">
          <strong>{t('drug_reference.interactions.footer_source_label')}</strong>{' '}
          {t('drug_reference.interactions.footer_source_text')}
        </footer>
      </div>
    </AppLayout>
  )
    ...
    <div className="flex items-center justify-between px-3 py-2 hover:bg-surface-secondary">
      ...
      <div className="min-w-0 flex-1 mr-3">
        <span className="text-sm font-medium text-text-primary block truncate">
          {result.brand_name ?? result.generic_name ?? t('drug_reference.interactions.unknown_drug')}
        </span>
      ...
      {selected ? (
        <span className="text-xs text-desert-green font-semibold flex-shrink-0">{t('drug_reference.interactions.added')}</span>
      ) : (
        <button
          type="button"
          onClick={onAdd}
        ...
        >
          {t('drug_reference.interactions.add')}
        </button>
      )}
    </div>
  )
