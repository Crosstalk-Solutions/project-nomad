...
export default function ChatInterface({
  ...
  const handleDownloadModel = async () => {
    ...
    try {
      ...
      addNotification({ type: 'success', message: t('chat.model_download_queued') })
    } catch (error) {
      addNotification({ type: 'error', message: t('chat.model_download_failed') })
    } finally {
        ...
        {messages.length === 0 ? (
          <div className="h-full flex items-center justify-center">
            <div className="text-center max-w-md">
              ...
              <h3 className="text-lg font-medium text-text-primary mb-2">{t('chat.start_conversation')}</h3>
              <p className="text-text-muted text-sm">
                {t('chat.interact_with_models')}
              </p>
              {chatSuggestionsEnabled &&
                ...
                !chatSuggestionsLoading && (
                  <div className="mt-8">
                    <h4 className="text-sm font-medium text-text-secondary mb-2">{t('chat.suggestions_label')}</h4>
              ...
              {chatSuggestionsEnabled && chatSuggestionsLoading && (
                <BouncingDots text={t('chat.thinking')} containerClassName="mt-8" />
              )}
              {!chatSuggestionsEnabled && (
                <div className="mt-8 text-sm text-text-muted">
                  {t('chat.enable_suggestions_hint')}
                </div>
              )}
            </div>
          </div>
      ...
      <div className="border-t border-border-subtle bg-surface-primary px-6 py-4 flex-shrink-0 min-h-[90px]">
        <form onSubmit={handleSubmit} className="flex gap-3 items-end">
          <div className="flex-1 relative">
            <textarea
              ...
              placeholder={t('chat.message_placeholder', { assistantName: aiAssistantName })}
        ...
        {!rewriteModelAvailable && (
          <div className="text-sm text-text-muted mt-2">
            {t('chat.model_not_installed', { model: DEFAULT_QUERY_REWRITE_MODEL })}{' '}
            <button
              onClick={() => setDownloadDialogOpen(true)}
              className="text-desert-green underline hover:text-desert-green/80 cursor-pointer"
            >
              {t('chat.downloading_it')}
            </button>{' '}
            {t('chat.rag_performance_suffix')}
          </div>
        )}
        <StyledModal
          open={downloadDialogOpen}
          title={t('chat.download_model_title', { model: DEFAULT_QUERY_REWRITE_MODEL })}
          confirmText={t('chat.download_confirm')}
          cancelText={t('chat.download_cancel')}
        ...
        >
          <p className="text-text-primary">
            {t('chat.download_description_prefix')}{' '}
            <span className="font-mono font-medium">{DEFAULT_QUERY_REWRITE_MODEL}</span>{' '}
            {t('chat.download_description_suffix')}
          </p>
        </StyledModal>
      </div>
    </div>
