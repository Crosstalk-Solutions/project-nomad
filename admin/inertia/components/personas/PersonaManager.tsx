...
function PersonaEditor({ detail }: { detail: PersonaDetail }) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-text-primary">{detail.label}</h3>
...
/**
 * Sidebar list + inline editor for managing persona system prompts.
 * Self-contained: drives its own list and detail queries via /api/personas.
 *
 * Rendered both as the body of /personas (full page) and as a section on
 * /settings/models (alongside the personas-enabled toggle).
 */
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 overflow-y-auto p-6">
        ...
        {detail && (
          // Keying on persona key remounts the form when the user picks a different
          // persona, which re-seeds the draft state without a useEffect → setState
          // ping-pong.
          <PersonaEditor key={detail.key} detail={detail} />
        )}
      </main>
    </div>
  )
