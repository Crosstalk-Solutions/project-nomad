...
export default function UpdateServiceModal({
    ...
    async function loadVersions() {
        ...
        try {
            ...
            showError(t('update.errorLoadVersions'))
        } finally {
    ...
    return (
        <StyledModal
            ...
            onCancel={onCancel}
            open={true}
            confirmText={t('update.confirmUpdate')}
        ...
        >
            <div className="space-y-4">
                ...
                <div>
                    ...
                    >
                        {showAdvanced ? t('update.hideVersions') : t('update.showVersions')}
                    </button>
                    ...
                    {showAdvanced && (
                        <>
                            <div className="mt-3 max-h-48 overflow-y-auto border rounded-lg divide-y">
                                {loadingVersions ? (
                                    <div className="p-4 text-center text-text-muted text-sm">{t('update.loadingVersions')}</div>
                                ) : versions.length === 0 ? (
                                    <div className="p-4 text-center text-text-muted text-sm">{t('update.noOtherVersions')}</div>
                                ) : (
                                    versions.map((v) => (
                                        <label
                                            key={v.tag}
