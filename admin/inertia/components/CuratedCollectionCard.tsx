  ...
  return (
    ...
    >
          ...
          {collection.all_installed && (
            <div className="flex items-center">
              <IconCircleCheck
                ...
                title={t('common.all_items_downloaded')}
              />
              <p className="text-lime-400 text-sm ml-1">{t('common.all_items_downloaded')}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <p className="text-gray-200 text-xs mt-2">
        {t('common.items_count', { count: collection.resources?.length })} | {t('common.size_label', { size: formatBytes(totalSizeBytes, 0) })}
      </p>
    </div>
  )
}
